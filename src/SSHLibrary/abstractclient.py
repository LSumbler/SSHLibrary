...
class _ClientConfiguration(Configuration):
    ...
    def __init__(self, host, alias, port, timeout, newline, prompt, term_type,
                 width, height, path_separator, encoding, proxy_uri):
        super(_ClientConfiguration, self).__init__(
            ...
            proxy_uri=StringEntry(proxy_uri)
        )
        logger.info('_ClientConfiguration.__init__:')
        logger.info('proxy_uri=%s' % proxy_uri)
